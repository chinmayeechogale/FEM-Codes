...
while ITER<=ITERMAX
    ...
    for N=1:NEM
        ...
        [ELK,ELF] = ELEMAT2D(DATA,SFL_ARRAY,DSFL_ARRAY,GAUSWT,NONLIN,...
                              ELXY,ELU);
...
function [ELK,ELF] = ELEMAT2D(DATA,SFL_ARRAY,DSFL_ARRAY,GAUSWT,NONLIN,...
                              ELXY,ELU)
    ...
    for NG = 1:NGP2
        ...
        F = FX0 + FX1*X + FY1*Y;
        if(NONLIN>0)
           A11 = A11 + A1U*U + A1UX*DUX + A1UY*DUY;
           A22 = A22 + A2U*U + A2UX*DUX + A2UY*DUY; 
        end
        
        % COMPUTE ELEMENT STIFFNESS MATRIX AND FORCE VECTOR
        ELK = ELK + A11*(GDSFL(:,1))*GDSFL(:,1)'*CNST + ...
